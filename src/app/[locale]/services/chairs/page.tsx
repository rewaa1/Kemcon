...
export default async function ChairsPage({ searchParams }: PageProps) {
  ...
  const schemas = [
     ...
     */
    {
      ...
      serviceType: isAr
        ? "تصميم الكراسي وتصنيعها وتنجيدها"
        : "Chair design, manufacture and upholstery",
      description: isAr
        ? "تصنيع الكراسي وتنجيدها حسب الطلب لمساحات الضيافة والتجارية والسكنية في مصر — هياكل من الخشب الصلب أو المعدن، وحشو بحسب الصلابة، ومعالجات اختيارية للقماش."
        : "Made-to-order chair manufacture and upholstery for hospitality, commercial and residential interiors in Egypt — hardwood or metal frames, filling by firmness, and optional fabric treatments.",
      url: pageUrl,
      inLanguage: locale,
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#1A1D24]">
        <ServiceIntroHeader category="chairs" locale={locale} editId={edit} />
        {/*
        ...
        */}
        {!edit && (
          <>
            <ServiceContent category="chairs" locale={locale} />
            <ServiceEnquiryHeading category="chairs" locale={locale} />
          </>
        )}
        <ErrorBoundary>
          <ProductEnquiryForm
