...
const seo = productSeo["bed-covers"];
...
export default async function BedCoversPage({ searchParams }: PageProps) {
  const locale = await getLocale();
  ...
  const schemas = [
     ...
     */
    {
      ...
      serviceType: isAr
        ? "تصنيع مفارش السرير والمخدات"
        : "Bed cover and pillow manufacture",
      description: isAr
        ? "مفارش سرير ومخدات مطابقة تُصنع بمقاس المرتبة للفنادق والمنشآت الصحية والمنازل في مصر، من القطن المصري والحرير والساتان وأقمشة أخرى، مع تشطيب صحي اختياري."
        : "Bed covers and matching pillows made to mattress size for hotels, healthcare and homes in Egypt, in Egyptian cotton, silk, satin and other fabrics, with optional hygienic finishing.",
      url: pageUrl,
      inLanguage: locale,
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#1A1D24]">
        <ServiceIntroHeader category="bed-covers" locale={locale} editId={edit} />
        {/*
        ...
        */}
        {!edit && (
          <>
            <ServiceContent category="bed-covers" locale={locale} />
            <ServiceEnquiryHeading category="bed-covers" locale={locale} />
          </>
        )}
        <ErrorBoundary>
          <ProductEnquiryForm
