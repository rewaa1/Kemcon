...
|----------|-------|--------|------|-----|
| ~~🔴 Critical~~ ✅ | ~~Hero LCP = 9.8s~~ — (1) CSS `backgroundImage` replaced with `<Image fill priority sizes="100vw">` → LCP dropped to 1.9s. (2) `motion.h1` `initial` changed from `{{ opacity: 0, y: 30 }}` to `{{ y: 20 }}` — h1 now visible on first paint so browser registers it as LCP candidate immediately; 0.5s opacity delay removed. | `src/components/sections/Hero.tsx` | Done |
...
| ~~🟡 Medium~~ ✅ | ~~Total Blocking Time — 300ms~~ → 660ms regression fixed — below-fold sections (`AboutPreview`, `ProductHighlights`, `WhyKemcon`, `CTABanner`) now use `ssr: false` + new `LazySection` wrapper (`src/components/shared/LazySection.tsx`). `LazySection` uses `IntersectionObserver` (rootMargin 400px) + `startTransition` to mount each section only as the user approaches it, spreading framer-motion IntersectionObserver registrations across scroll time instead of bursting at initial paint. | `src/app/[locale]/page.tsx`, `src/components/shared/LazySection.tsx` | Done |
| ~~🟡 Medium~~ ✅ | ~~Legacy JavaScript — 14 KiB savings~~ — `browserslist` added to `package.json` targeting Chrome/Edge ≥93, Firefox ≥92, Safari ≥15.4. Eliminates polyfills for `Array.at`, `Array.flat`, `Object.fromEntries`, `Object.hasOwn`, `String.trimStart/End` — all baseline features in every modern browser since 2021. | `package.json` | Done |
