...
| 🟠 High | Render-blocking requests — 570ms | Synchronous resource loading blocking first paint | Check Sentry browser script injection and any non-deferred third-party tags | Defer or async all non-critical scripts |
| 🟠 High | Unused JavaScript — 91 KiB savings | Framer Motion and Lenis bundle loaded upfront even for elements not yet in viewport | `src/components/sections/Hero.tsx`, `src/app/[locale]/layout.tsx` | Lazy-load below-fold animation sections with `dynamic(() => ..., { ssr: false })` |
...
| 🟠 High | Sentry DSN hardcoded in `sentry.server.config.ts` and `sentry.edge.config.ts` | Both files, line 8–9 | Use `process.env.NEXT_PUBLIC_SENTRY_DSN` like the client config already does |
